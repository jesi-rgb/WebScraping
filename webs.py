...
def extract():
    ...
    for l in full_links:
        ...
        lista_prestaciones = div_prestaciones.find_all(
            'span', attrs={'class': 'cc_data'})

        for p in lista_prestaciones:
            categoria = p.find_all('span', attrs={'class': 'cc_label'})
            valor = p.find_all('strong')

            for c, valor in zip(categoria, valor):
                prestaciones[c] = v
    ...
    datos.to_csv('products.csv', index=False, encoding='utf-8')
